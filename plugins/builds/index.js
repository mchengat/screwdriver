...
const buildsPlugin = {
    ...
    async register(server, options) {
        ...
        server.expose('removeJoinChild', async (config, app) => {
            ...
            const triggerFetchBuild = async (nextJobName, joinObj) => {
                ...
                return getInternalBuild(internalBuildConfig);
            };
            ...
            for (const pid of Object.keys(pipelineJoinData)) {
                if (+pid === current.pipeline.id) {
                    for (const nextJobName of Object.keys(pipelineJoinData[pid].jobs)) {
                        try {
                            const buildToDel = await triggerFetchBuild(nextJobName, pipelineJoinData[pid].jobs);
